...
void main(void) {
    ...
    for (;;) {
        ...
        if (rc >= 0) {
            rc = backend_write("scope.txt", "no", 2U);
            if (rc == OS_VFS_BACKEND_DENIED) {
                puts("vfsreadclaim read-only enforced\n");
                for (;;) yield();
            }
            puts("vfsreadclaim mutation unexpectedly allowed\n");
            for (;;) yield();
        }
        if (!announced) { puts("vfsreadclaim waiting read\n"); announced = 1; }
        yield();
    }
}
