 ...
 * LBA 2000 lies past the 64-sector overlay snapshot (LBA 0-63). */
...
void main(void) {
    ...
    rc = ata_call(driver, OS_IPC_ATA_WRITE, 1U, SCRATCH_LBA, pattern, 16U, &reply);
    if (rc != 16) { puts("ataclient write failed\n"); for (;;) yield(); }
    rc = ata_call(driver, OS_IPC_ATA_READ, 2U, SCRATCH_LBA, 0, 16U, &reply);
    ok = rc == 16;
