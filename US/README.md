...
| **Instance OS autonome** | [mohhdy_agent_support_web.md](mohhdy_agent_support_web.md) + [mohhdy_os_ui_migration.md](mohhdy_os_ui_migration.md) | Backlog de **capacites OS** (`ASSIST-xxx`) portees dans le bootstrap graphique `osui/` du **meme** SE Multiboot (chat central, scene IA, slash, shell Ring 3 ; OS-UI-0/1/2 premieres tranches) avec scaffold `agent/` temporaire. Stub local. Playwright = profil optionnel. LLM de production, US-031 et billing **non livres**. Le guest n'a pas `#ai-stage`. Prochain : OS-UI-3 |
