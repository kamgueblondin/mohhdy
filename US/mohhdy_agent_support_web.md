...
Comportement attendu (spec, non livré) :

1. Le script dessine un lanceur, puis un panneau de conversation.
2. Il ouvre une session visiteur vers l'origine de l'instance (self-host ou cloud).
3. Les messages transitent en HTTPS. Les actes (clics, MCP) partent de l'agent **côté instance**, pas du navigateur du visiteur comme backdoor.
4. Si l'opérateur a autorisé des actes sur **son** origine, l'agent les exécute dans un navigateur outillé de l'instance, ou via l'outil MCP déclaré, jamais en élargissant l'origine tout seul.
5. Un bouton "parler à un humain" déclenche `session.escalate`.

Scripts d'intégration (ASSIST-013) : snippet, vérification d'origine, CSP recommandée, et mode "attacher à n'importe quel site" sous réserve que l'opérateur contrôle ce site.

## Console admin et handoff

La console n'est pas le shell VGA. C'est une UI web de l'instance.

- Liste des sessions (ouvertes, en attente humaine, closes).
- Lecture du fil, des actes tentés, des refus de capacité.
