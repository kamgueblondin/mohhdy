...
| 2 | Topologie réseau locale partagée + TLS multi + guest-guest | `make qemu-ne2k-shared-topology` + `make qemu-ne2k-tls-multi-guest` + `make qemu-ne2k-guest-app-traffic` (hub 127.0.0.1, dual lease, TLS_COMPLETE x2, ARP croise / SYN peer.local, hors ci) ; sans TAP, clé, Internet public ni OpenAI |
