...
| `make qemu-net-worker` | Tranche 5 : avec `net-driver` enregistre, syscalls peer/LLM reseau refuses (-59) hors PID worker, `net-status` reste lisible ; slice 2 : syscalls socket 99-108 des autres taches relayes au worker par IPC (`netrelay` 14 appels, compteurs `net-relay-status`, timeout -87, abort -88) ; mode degrade inchange (pilote NE2000 et registre TCP toujours Ring 0) |
...
| `make run` / `make run-gui` | `make run` = QEMU curses. `make run-gui` = fenetre graphique QEMU (VBE suit la fenetre). Apres `MOHHDY>`, tapez `gui`. `console` / ESC revient au texte |
| `make osui-smoke` | Registre guest + facade Python absente. Inclus dans `make test-all` |
...
La surface OS-UI vit dans le shell Ring 3 : `userspace/osui_runtime.c` (cerveau) + `kernel/gfx_desktop.c` (bureau VBE dans la fenetre QEMU, suit la taille de la fenetre). `make run-gui` lance QEMU GTK. Apres `MOHHDY>`, tapez `gui` (aliases `graphics`, `desktop`). Slash `/help` `/browser` `/shell` `/admin` `/support` `/status` `/fs` `/plan` `/center`. Quand un programme s'ouvre, le chat passe en `chat_mode=float` ; `/center` le ramene au centre. Pont : `shared/multiboot_shell_commands.json`. Les reponses chat sont un stub local (`llm=stub_echo`), **pas** un LLM de production. Les gestes sont un simulateur DOM, **pas** Chromium, **pas** US-031. `agent/` et le serveur metier Python `osui/server.py` sont **retires**. Ce n'est **pas** une fenetre HTML hote. Guides : [docs/osui_0_1_2.md](docs/osui_0_1_2.md), [docs/osui_chat_desktop.md](docs/osui_chat_desktop.md), [docs/osui_ai_stage.md](docs/osui_ai_stage.md), [docs/osui_shell_live.md](docs/osui_shell_live.md), [docs/osui_convergence.md](docs/osui_convergence.md).

```bash
make osui-smoke
make qemu-osui-runtime
make qemu-osui-gui
make run-gui          # QEMU GTK, bureau VBE ; tapez gui apres MOHHDY>
make osui-docker
...
- [~] Migration microkernel réelle : `vfsserver` délègue à `vfsvirtual` Ring 3 les vues publiques, les mutations fixes, les I/O des alias dynamiques **et** les I/O `read`/`stat`/liste/pages/observation des montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) sous capacité backend temporaire droit-source-préfixe relatif, corrélation PID/requête et absence de rejeu ([docs/aos2163_2170_vfs_storage_worker_separation.md](docs/aos2163_2170_vfs_storage_worker_separation.md)) ; AOS-2172 ferme le bypass propriétaire FAT16/FAT32 tant que `vfs-virtual` est publié ; AOS-2173 ferme le même bypass pour initrd/overlay ; AOS-2174 ferme le bypass SOURCE_ALL / combinaisons ATA-backed ; AOS-2175 restreint overlay read/stat au PID `vfs-virtual` ; AOS-2177 gate `SYS_READFILE`/`SYS_WRITEFILE` historiques sur la partie overlay (initrd RAM reste lisible) ; AOS-2178 ferme les autres points d'entree historiques overlay (`SYS_STAT`, `SYS_LISTDIR`, `SYS_MKDIR`, `SYS_UNLINK`, `SYS_RENAME`, `SYS_COPY`, `SYS_APPEND`) et la liste overlay backend ; la séparation complète du pilote ATA/FAT hors noyau reste ouverte
- [~] Tranche 4, premier pas du pilote de stockage Ring 3 : `atadriver` accede aux ports ATA 0x1F0-0x1F7/0x3F6 a CPL 3 via le bitmap I/O du TSS (ports refuses a toute autre tache, #GP qui ne tue que la tache fautive) et sert des fenetres de secteur de 64 octets par IPC au seul `ataclient` ; tranche 4 slice 2 : quand le pilote est vivant, le flush du snapshot overlay (LBA 0-63) passe par lui en blocs de 8 secteurs copies par le noyau (`SYS_ATA_JOB_FETCH/DONE`) et un chargement post-demarrage relit le snapshot via son PIO Ring 3 ; un claim (`SYS_ATA_CLAIM`) ouvre les ports et fait refuser le PIO noyau (-85) pendant la fenetre ; les ecritures client sont refusees sur l'overlay et les volumes FAT ; tranche 4 slice 3 : le noyau lance `atadriver` au boot quand un disque IDE est present (chemin pilote par defaut) et les E/S secteur FAT16/FAT32 passent par lui quand il est vivant (RPC synchrone : la tache appelante est bloquee dans son syscall pendant que le pilote fait le PIO Ring 3 ; compteur PIO noyau FAT a 0 pendant ce temps, prouve en QEMU). Le PIO noyau Ring 0 reste utilise au boot (chargement overlay, montage FAT, avant toute tache) et comme repli quand le pilote est absent ou mort (prouve apres `kill`) ; la logique FAT/overlay reste dans le noyau ([docs/tranche4_ata_ring3_driver.md](docs/tranche4_ata_ring3_driver.md), `make qemu-ata-driver`)
...
- [x] CI GitHub sous 25 minutes de bout en bout : les sept contrats tournent en deux jobs paralleles (`make integration-qemu-vfs` = vfs-service, `make integration-qemu-rest` = les six autres + `qemu-ne2k-tls-multipair`), sans dependance sur `build-and-test` ; une etape verifie que l'union des deux shards vaut exactement les sept contrats. Mesure reelle : 25-26 min avant (runs 36249178581, 36255587009), 10 min 24 s apres (run 36262363929), aucun contrat ni assertion retire
...
- [~] Tranche 5 isolation reseau : avec `networker` (`net-driver`) enregistre, les syscalls LLM reseau (91-98), socket (99-108) et peer (128-130) sont reserves a son PID (`OS_NET_WORKER_REQUIRED` -59) ; slice 2 : les syscalls socket (99-108) des autres taches sont relayes au worker par IPC (codec de segments TCP, pas d'emission NIC ; LLM/peer toujours -59) ; pilote NE2000 et piles TCP/TLS encore Ring 0 ([docs/tranche5_net_worker_gate.md](docs/tranche5_net_worker_gate.md))
...
- [x] Préalable PS/2 multi-QEMU (Garde 2) : `make qemu-ps2-dual` prouve deux invités TCG simultanés avec injection `sendkey` mutexée côté hôte
- [x] Topologie Ethernet locale partagée (tranche 2) : `make qemu-ne2k-shared-topology` + `make qemu-ne2k-tls-multi-guest` + `make qemu-ne2k-guest-app-traffic` + `make qemu-ne2k-guest-tls-peer` + `make qemu-ne2k-guest-tls-chat` + `make qemu-ne2k-guest-tls-server` — hub `127.0.0.1`, dual lease, TLS_COMPLETE ×2, ARP / SYN / SYN-ACK guest / ACK final + ClientHello / serveur TLS Finished (hors ci)
