...
- [x] Topologie Ethernet locale partagée (tranche 2) : `make qemu-ne2k-shared-topology` + `make qemu-ne2k-tls-multi-guest` + `make qemu-ne2k-guest-app-traffic` + `make qemu-ne2k-guest-tls-peer` + `make qemu-ne2k-guest-tls-chat` — hub `127.0.0.1`, dual lease, TLS_COMPLETE ×2, ARP / SYN / SYN-ACK guest / ACK final + ClientHello TLS (hors ci)
