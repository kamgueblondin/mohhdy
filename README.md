...
| `make qemu-ne2k-guest-tls-appdata` | Suite : ping/pong AES-GCM guest-guest (hors `make ci`) |
| `make qemu-ne2k-guest-tls-chat-metier` | Suite : chat metier AES-GCM bilaterale >=2/sens (hors `make ci`) |
...
- [x] Topologie Ethernet locale partagée (tranche 2) : `make qemu-ne2k-shared-topology` + `make qemu-ne2k-tls-multi-guest` + `make qemu-ne2k-guest-app-traffic` + `make qemu-ne2k-guest-tls-peer` + `make qemu-ne2k-guest-tls-chat` + `make qemu-ne2k-guest-tls-server` + `make qemu-ne2k-guest-tls-appdata` + `make qemu-ne2k-guest-tls-chat-metier` — hub `127.0.0.1`, dual lease, TLS_COMPLETE ×2, ARP / SYN / SYN-ACK guest / ACK final + ClientHello / serveur TLS Finished / ping-pong + chat metier AES-GCM (hors ci)
