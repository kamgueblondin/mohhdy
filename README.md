...
- [~] Migration microkernel réelle : `vfsserver` délègue à `vfsvirtual` Ring 3 les vues publiques, les mutations fixes, les I/O des alias dynamiques **et** les I/O `read`/`stat`/liste/pages/observation des montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) sous capacité backend temporaire droit-source-préfixe relatif, corrélation PID/requête et absence de rejeu ([docs/aos2163_2170_vfs_storage_worker_separation.md](docs/aos2163_2170_vfs_storage_worker_separation.md)) ; AOS-2172 ferme le bypass propriétaire FAT16/FAT32 tant que `vfs-virtual` est publié ; AOS-2173 ferme le même bypass pour initrd/overlay ; AOS-2174 ferme le bypass SOURCE_ALL / combinaisons ATA-backed ; AOS-2175 restreint overlay read/stat au PID `vfs-virtual` ; AOS-2176 etend list/mutate overlay au meme PID ; la séparation complète du pilote ATA/FAT hors noyau reste ouverte
...
- [x] Topologie Ethernet locale partagée (tranche 2) : `make qemu-ne2k-shared-topology` + `make qemu-ne2k-tls-multi-guest` + `make qemu-ne2k-guest-app-traffic` — hub `127.0.0.1`, dual lease, TLS_COMPLETE ×2, ARP croisé / SYN `peer.local` (hors ci)
