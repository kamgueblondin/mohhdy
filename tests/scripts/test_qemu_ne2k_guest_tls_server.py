#!/usr/bin/env python3
"""Guest↔guest : role serveur TLS B jusqu a Finished (+ app echo).

Suite apres guest-tls-chat. Deux QEMU TCG partagent
SharedEthernetHub(proxy_peer_syn_ack=False). Apres ESTABLISHED + ClientHello,
B joue le role serveur TLS 1.2 in-tree (ServerHello..Finished AES-GCM) via
ai-peer-tls-poll ; A complete via ai-tls-poll jusqu a TLS_COMPLETE. Au moins
un record applicatif chiffre dans chaque sens (ping/pong). Hub ne fake pas
la crypto. Hors make ci. Sans TAP, Internet, secret ni OpenAI.
"""
...
INSTANCES = (
    {
        ...
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-server-a.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-server-a.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-server-a.monitor.sock"),
    },
    {
        "label": "b",
        "mac": "52:54:00:a0:20:0b",
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-server-b.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-server-b.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-server-b.monitor.sock"),
    },
)
...
def main():
    ...
    try:
        ...
        say(
            ...
            "hub proxy off; events=%s leases=%s)."
            % (
                hub.events,
                {k: ".".join(str(b) for b in v) for k, v in hub.leases.items()},
            )
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("QEMU NE2000 guest-guest TLS server step failed: %s\n" % error)
        raise SystemExit(1)
