#!/usr/bin/env python3
"""Guest↔guest : B ecoute et emet un vrai SYN-ACK (pas de proxy hub).

Deux QEMU TCG partagent SharedEthernetHub(proxy_peer_syn_ack=False). B prend
un bail, ecoute :443, A resout peer.local et envoie SYN. B accepte le SYN et
emet le SYN-ACK depuis l invite (guest_syn_ack). Pas encore de TLS metier.
Hors make ci / integration-qemu. Sans TAP, Internet, secret ni OpenAI.
"""
...
INSTANCES = (
    {
        ...
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-a.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-a.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-a.monitor.sock"),
    },
    {
        "label": "b",
        "mac": "52:54:00:a0:20:0b",
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-b.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-b.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-peer-b.monitor.sock"),
    },
)
...
def main():
    ...
    hub.start()
    say("[guest-tls-peer] hub listening on 127.0.0.1:%d" % hub.port)
    ...
    try:
        ...
        for inst in INSTANCES:
            ...
            say("[guest-tls-peer] guest %s booted" % inst["label"])
        ...
        say("[guest-tls-peer] guest B leased; leases=%r" % (
        ...
        say(
            "QEMU NE2000 guest-guest TLS-peer step passed "
            "(B LISTEN + guest SYN-ACK, hub proxy off, peer.local DNS, "
            "cross ARP + SYN, mutexed PS/2; events=%s leases=%s)."
            % (
                hub.events,
                {k: ".".join(str(b) for b in v) for k, v in hub.leases.items()},
            )
