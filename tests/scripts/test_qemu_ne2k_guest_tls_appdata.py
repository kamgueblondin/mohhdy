#!/usr/bin/env python3
"""Guest↔guest : application data AES-GCM dans les deux sens.

Suite apres guest-tls-server. A complete le handshake puis emet un record
applicatif chiffre "ping". B l'ouvre et repond par un record chiffre "pong".
Le hub ne fake pas la crypto et compte au moins un record 0x17 dans chaque
sens. Hors make ci. Sans TAP, Internet, secret ni OpenAI.
"""
...
INSTANCES = (
    {
        ...
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-a.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-a.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-a.monitor.sock"),
    },
    {
        "label": "b",
        "mac": "52:54:00:a0:20:0b",
        "log": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-b.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-b.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-tls-appdata-b.monitor.sock"),
    },
)
...
def main():
    ...
    try:
        ...
        for inst in INSTANCES:
            ...
            say("[guest-tls-server] guest %s booted" % inst["label"])
        ...
        say("[guest-tls-server] guest B leased; leases=%r" % (
        ...
        # A: record applicatif AES-GCM "ping" apres TLS_COMPLETE.
        run_command(
            guest_a, "ai-app-ping", "ai-app-ping: app ping emis",
            peer_alive_check, timeout=30,
        )
        deadline = time.monotonic() + 10
        while time.monotonic() < deadline and hub.events.get("guest_app_data_c2s", 0) < 1:
            time.sleep(0.1)
        if hub.events.get("guest_app_data_c2s", 0) < 1:
            raise RuntimeError(
                "expected encrypted app data A->B; events=%r" % hub.events
            )

        # B ouvre "ping" et repond "pong" avec sa session AES-GCM.
        echoed = False
        for _ in range(24):
            start_b = run_command(
                guest_b, "ai-peer-tls-poll", "ai-peer-tls-poll:",
            ...
            if "ai-peer-tls-poll: echec" in chunk_b:
                raise RuntimeError(
                    "peer app echo failed on B; events=%r log=%s"
                    % (hub.events, chunk_b[-1200:])
                )
            if "app echo emis" in chunk_b:
                echoed = True
        ...
        if not echoed:
            raise RuntimeError(
                "expected B app echo serial marker; events=%r" % hub.events
            )
        deadline = time.monotonic() + 10
        while time.monotonic() < deadline and hub.events.get("guest_app_data_s2c", 0) < 1:
            time.sleep(0.1)
        if hub.events.get("guest_app_data_s2c", 0) < 1:
            raise RuntimeError(
                "expected encrypted app data B->A; events=%r" % hub.events
            )
        if hub.events.get("guest_app_data", 0) < 2:
            raise RuntimeError(
                "expected at least two guest app records total; events=%r" % hub.events
            )
        ...
        say(
            "QEMU NE2000 guest-guest TLS appdata passed "
            "(A app ping AES-GCM, B app pong AES-GCM, "
            "guest_app_data_c2s>=1, guest_app_data_s2c>=1, hub proxy off; "
            "events=%s leases=%s)."
            % (
                hub.events,
                {k: ".".join(str(b) for b in v) for k, v in hub.leases.items()},
