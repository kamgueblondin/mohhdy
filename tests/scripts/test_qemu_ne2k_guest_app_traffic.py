#!/usr/bin/env python3
"""Guest↔guest ARP croise + flux applicatif simple sur hub Ethernet partage.

Deux QEMU TCG simultanés partagent SharedEthernetHub. L invite B prend un bail
DHCP via ai-acquire example.com. L invite A resout peer.local (IP de B), emet
un ARP croise puis un SYN TCP vers B:443. Le hub proxy ARP/SYN-ACK du pair.
Hors make ci / integration-qemu. Sans TAP, Internet public, secret ni OpenAI.
"""
...
INSTANCES = (
    {
        ...
        "log": os.path.join(LOG_DIR, "ne2k-guest-app-a.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-app-a.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-app-a.monitor.sock"),
    },
    {
        "label": "b",
        "mac": "52:54:00:a0:20:0b",
        "log": os.path.join(LOG_DIR, "ne2k-guest-app-b.log"),
        "err": os.path.join(LOG_DIR, "ne2k-guest-app-b.err"),
        "mon": os.path.join(LOG_DIR, "ne2k-guest-app-b.monitor.sock"),
    },
)
...
def main():
    ...
    hub = SharedEthernetHub(respond=True, full_tls=False)
    hub.start()
    say("[guest-app] hub listening on 127.0.0.1:%d" % hub.port)
    ...
    try:
        ...
        for inst in INSTANCES:
            ...
            say("[guest-app] guest %s booted" % inst["label"])
        ...
        say("[guest-app] guest B leased; leases=%r" % (
        ...
        if hub.events["cross_syn"] < 1 or hub.events["cross_syn_ack"] < 1:
            raise RuntimeError(
                "expected cross-guest SYN stream; events=%r" % hub.events
            )
        if len(hub.leases) < 2:
            raise RuntimeError("expected 2 DHCP leases; got %r" % hub.leases)

        peer_alive_check()
        # B still answers on the shared segment after A peer traffic.
        run_command(
        ...
        say(
            "QEMU NE2000 guest-guest app traffic passed "
            "(2 simultaneous guests, dual DHCP leases, peer.local DNS, "
            "cross ARP + SYN stream, mutexed PS/2; events=%s leases=%s)."
            % (
                hub.events,
                {k: ".".join(str(b) for b in v) for k, v in hub.leases.items()},
            )
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("QEMU NE2000 guest-guest app traffic failed: %s\n" % error)
        raise SystemExit(1)
