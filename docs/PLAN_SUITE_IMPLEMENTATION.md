...
Suite : `make qemu-ne2k-tls-multi-guest` (TLS_COMPLETE x2), `make qemu-ne2k-guest-app-traffic` (ARP croise + SYN peer.local, hors ci), `make qemu-ne2k-guest-tls-peer` (B LISTEN + SYN-ACK guest, hors ci), puis `make qemu-ne2k-guest-tls-chat` (ACK final + ClientHello TLS, hors ci), puis `make qemu-ne2k-guest-tls-server` (role serveur TLS Finished, hors ci), puis `make qemu-ne2k-guest-tls-appdata` (ping/pong AES-GCM, hors ci).
...
| 2 | Topologie locale partagee | Prototype guest | `qemu-ne2k-shared-topology` + `qemu-ne2k-tls-multi-guest` + `qemu-ne2k-guest-app-traffic` + `qemu-ne2k-guest-tls-peer` + `qemu-ne2k-guest-tls-chat` + `qemu-ne2k-guest-tls-server` + `qemu-ne2k-guest-tls-appdata` (hub 127.0.0.1, hors ci) |
