...
| capacité | `layers x max_positions x 2 x channels` floats exigés |
...
La fixture alloue un cache `2 x 3 x 2 x 4`, écrit et relit les K/V de la couche 1 à la position 2, vérifie `count == 3`, puis rejette une couche hors bornes, une capacité de stockage insuffisante et un pointeur nul. `make test-all` passe avec **265 tests réussis, 0 échec et 0 test ignoré**.
