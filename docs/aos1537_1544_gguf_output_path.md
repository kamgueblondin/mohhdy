...
Les deux fonctions exigent des buffers caller-owned. Les capacités de sortie sont exprimées en octets, comme les primitives de projection existantes. Les tailles `largeur x sizeof(float)` sont contrôlées avant toute multiplication afin d'empêcher un contournement par débordement sur des métadonnées GGUF non fiables.
