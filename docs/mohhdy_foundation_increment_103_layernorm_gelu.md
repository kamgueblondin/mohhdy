...
`gpt2_gguf_layernorm` calcule la moyenne et la variance en deux passes, applique l'inverse de la racine carrée freestanding déjà utilisée par l'attention, puis produit `((x - moyenne) / sqrt(variance + epsilon)) x gamma + beta`. Les paramètres `gamma`, `beta`, `epsilon` et la destination sont fournis par l'appelant. Les dimensions nulles, epsilon non positif, pointeurs nuls et capacité insuffisante sont rejetés.

## GELU

`gpt2_gguf_gelu` utilise l'approximation freestanding `x x sigmoid(1,702x)`, dont l'exponentielle est bornée par le noyau d'attention. Elle évite `libm`, ne modifie pas l'entrée et écrit dans un buffer caller-owned.
