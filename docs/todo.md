...
Le noyau contient désormais une arithmétique bigint caller-owned multi-limb, une réduction modulaire binaire, une exponentiation publique et `rsa_pkcs1_v15_sha256_verify`. La vérification exige un workspace de `7 x ceil(taille_module/4)` limbs fourni et aligné par l'appelant ; elle ne recourt à aucune allocation dynamique. Les tests couvrent une signature RSA-512 PKCS#1 v1.5 SHA-256 valide, les rejets de digest et de signature falsifiés, ainsi qu'une exponentiation à deux limbs.
