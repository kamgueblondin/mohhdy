...
echo "${shell}" | grep -q 'id="ai-stage"' || fail "scene IA"
echo "${shell}" | grep -q 'data-testid="ai-stage"' || fail "scene IA testid"
echo "${shell}" | grep -q 'id="ai-stage-mode"' || fail "mode scene IA"
...
echo "${shell}" | grep -q 'data-pane="shell"' || fail "pane Shell Multiboot"
echo "${shell}" | grep -q 'data-pane="fs"' || fail "pane FS"
echo "${shell}" | grep -q 'MOHHDY' || fail "prompt Multiboot"
echo "${shell}" | grep -q 'userspace/shell.c' || fail "mapping guest shell"
...
echo "${health}" | grep -q 'userspace/shell.c' || fail "source guest shell"
...
grep -q '.ai-stage' /tmp/mohhdy-osui-smoke-css || fail "css ai-stage"

stage="$(curl -fsS -X POST -H 'Content-Type: application/json' \
  -d '{"prompt":"dessine trois boites"}' "${BASE_URL}/api/os/stage")"
echo "${stage}" | grep -q 'stub_echo' || fail "stage llm stub"
echo "${stage}" | grep -q 'presenting' || fail "stage mode presenting"
echo "${stage}" | grep -q 'ai-scene' || fail "stage html scene"
echo "${stage}" | grep -q '<svg' || fail "stage svg"
echo "${stage}" | grep -q '<rect' || fail "stage rect"
...
echo "${shell_help}" | grep -q 'vfs-list' || fail "shell help vfs-list"
echo "${shell_help}" | grep -q 'ai' || fail "shell help ai"
echo "${shell_help}" | grep -q 'MOHHDY>' || fail "shell prompt"

shell_trap="$(curl -fsS -X POST -H 'Content-Type: application/json' \
  -d '{"line":"apt install nginx"}' "${BASE_URL}/api/os/shell")"
echo "${shell_trap}" | grep -q 'Pas un bash Linux' || fail "shell refuse bash"
